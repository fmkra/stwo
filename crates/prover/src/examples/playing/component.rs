...
impl FibonacciComponent {
    ...
    // fn step_constraint_eval_quotient_by_mask_1<F: ExtensionOf<BaseField>>(
    //     &self,
    //     point: CirclePoint<F>,
    //     mask: &[F; 4],
    // ) -> F {
    //     let constraint_zero_domain = Coset::subgroup(self.log_size - 1);
    //     // let constraint_value = mask[2] - mask[0] - F::from(BaseField::from(1));
    //     let constraint_value = mask[0] - mask[2] - F::one();
    ...
    //     num / denom
    // }
    // fn step_constraint_eval_quotient_by_mask_2<F: ExtensionOf<BaseField>>(
    //     &self,
    //     point: CirclePoint<F>,
    //     mask: &[F; 4],
    // ) -> F {
    ...
    fn step_constraint_eval_quotient_by_mask<F: ExtensionOf<BaseField>>(
        ...
        let constraint_zero_domain = Coset::subgroup(self.log_size - 1);
        // let constraint_value = mask[2] - mask[0] - F::from(BaseField::from(1));
        let constraint_value = (mask[0] - mask[2] - F::one()) + F::from(BaseField::from(1251)) * (mask[0] * mask[1] - mask[3]);
        let selector = point_excluder(
            constraint_zero_domain
                .at(constraint_zero_domain.size() - 1)
                .into_ef(),
    ...
    fn boundary_constraint_eval_quotient_by_mask<F: ExtensionOf<BaseField>>(
        ...
        let linear = F::one() + (g_minus_1.into_ef() + point).y * (self.claim - BaseField::one()) * g_minus_2.y.inverse();
...
impl Component for FibonacciComponent {
    ...
    fn evaluate_constraint_quotients_at_point(
        ...
        // evaluation_accumulator.accumulate(
        //     self.step_constraint_eval_quotient_by_mask_1(point, &mask[0][0][..].try_into().unwrap()),
        // );
        // evaluation_accumulator.accumulate(
        //     self.step_constraint_eval_quotient_by_mask_2(point, &mask[0][0][..].try_into().unwrap()),
        // );
    ...
    fn evaluate_constraint_quotients_on_domain(
        ...
        for (off, point_coset) in [
            ...
            for (i, point) in point_coset.iter().enumerate() {
                ...
                // res += self.step_constraint_eval_quotient_by_mask_1(point, &mask)
                //     * accum.random_coeff_powers[1];
                // res += self.step_constraint_eval_quotient_by_mask_2(point, &mask)
                //     * accum.random_coeff_powers[2];
